...
//
// The code in this lesson is identical to Lesson 04, but we explore the full pipeline:
//
//   [Application] --OTLP/gRPC--> [OTel Collector] --[exporters]--> [Backends]
//
// The Collector pipeline consists of:
//   1. RECEIVERS  - Accept OTLP data (gRPC on port 4317, HTTP on 4318)
//   2. PROCESSORS - Transform/filter data (batch, memory_limiter, attributes)
//   3. EXPORTERS  - Send to backends (debug, otlphttp to Grafana/Tempo/Loki)
//
// In docker-compose.yml, the collector config routes:
//   - traces  --> Tempo (port 4317 for otlphttp)
//   - metrics --> Prometheus (scraped via /metrics endpoint)
//   - logs    --> Loki (via otlphttp exporter)
//
// The application doesn't need to know about these backends. It only speaks OTLP
// to localhost:4317. The Collector handles all routing, batching, and retries.
...
async fn main() {
    ...
    startup.add_attribute("lesson", "05-collector-pipeline");
...
async fn get_task(
    ...
    Json(serde_json::json!({
        ...
        "title": "example task",
        "done": false,
        "note": "collector pipeline - otlp -> collector -> backends"
    }))
}
